...
required=[Path('frontend/src/main.tsx'),Path('frontend/src/fiberMap.tsx'),Path('backend/app/modules/uisp/client.py'),Path('backend/app/modules/tauc/client.py'),Path('compose.rc1.yml'),Path('.gitignore'),Path('deploy/preflight-rc1.sh'),Path('deploy/release-current.sh')]
for p in required:
...
assert 'secrets/' in ignore and '*.key' in ignore and 'backups/' in ignore
...
PY
bash -n deploy/preflight-rc1.sh
bash -n deploy/release-current.sh
