...
export function AccessControl({
  ...
  useEffect(() => {
    setUserRoles(Object.fromEntries(access.users.map(user => [user.id, user.roles])));
    setRolePermissions(Object.fromEntries(access.roles.map(role => [role.id, role.permissions])));
  }, [access]);
  ...
  return <div className="access-control">
    ...
    <section>
      ...
      <div className="access-grid">{access.users.map(user => <article key={user.id}>
        <div className="access-title"><div><strong>{user.display_name}</strong><small>{user.email}</small></div><span className={user.is_active ? "access-active" : "access-inactive"}>{user.is_active ? "Active" : "Disabled"}</span></div>
        <div className="access-options">{access.roles.map(role => <label key={role.id}><input type="checkbox" checked={(userRoles[user.id] || []).includes(role.name)} onChange={() => setUserRoles({ ...userRoles, [user.id]: toggle(userRoles[user.id] || [], role.name) })} /><span>{role.name.replaceAll("_", " ")}</span></label>)}</div>
    ...
    <section>
      ...
      <div className="access-grid">{access.roles.map(role => <article key={role.id}>
        ...
        <div className="permission-options">{access.permissions.map(permission => <label key={permission.code} title={permission.description}><input type="checkbox" checked={(rolePermissions[role.id] || []).includes(permission.code)} onChange={() => setRolePermissions({ ...rolePermissions, [role.id]: toggle(rolePermissions[role.id] || [], permission.code) })} /><span>{permission.code}</span></label>)}</div>
